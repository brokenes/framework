package com.github.framework.sensitive.core.support.filter;
...
import com.github.framework.sensitive.core.util.condition.SensitiveConditions;
...
public class DefaultContextValueFilter implements ContextValueFilter {
    ...
    public DefaultContextValueFilter(final SensitiveContext context) {
        this.sensitiveContext = context;
    }

    @Override
    public Object process(final BeanContext context, final Object object, final String name, final Object value) {
        ...
        // 设置当前处理的字段
        final SensitiveEntry sensitiveEntry = field.getAnnotation(SensitiveEntry.class);
        ...
        if(ClassTypeUtil.isArray(fieldTypeClass)) {
            ...
            if (ArrayUtil.isNotEmpty(arrays)) {
                final Object firstArrayEntry = ArrayUtil.firstNotNullElem(arrays).get();
                final Class entryFieldClass = firstArrayEntry.getClass();

                if(isBaseType(entryFieldClass)) {
                    //2, 基础值，直接循环设置即可
                    final int arrayLength = arrays.length;
                    final Object newArray = Array.newInstance(entryFieldClass, arrayLength);
                    for (int i = 0; i < arrayLength; i++) {
                        final Object entry = arrays[i];
                        sensitiveContext.setEntry(entry);
                        final Object result = handleSensitive(sensitiveContext, field);
        ...
        if(ClassTypeUtil.isCollection(fieldTypeClass)) {
            ...
            if (CollectionUtil.isNotEmpty(entryCollection)) {
                final Object firstCollectionEntry = CollectionUtil.firstNotNullElem(entryCollection).get();

                if(isBaseType(firstCollectionEntry.getClass())) {
                    //2, 基础值，直接循环设置即可
                    final List<Object> newResultList = new ArrayList<>(entryCollection.size());
                    for (final Object entry : entryCollection) {
                        sensitiveContext.setEntry(entry);
                        final Object result = handleSensitive(sensitiveContext, field);
    ...
    private Object handleSensitive(final SensitiveContext context,
        ...
        try {
            ...
            if (ObjectUtil.isNotNull(sensitive)) {
                final Class<? extends ICondition> conditionClass = sensitive.condition();
                final ICondition condition = conditionClass.newInstance();
                if (condition.valid(context)) {
                    final Class<? extends IStrategy> strategyClass = sensitive.strategy();
                    final IStrategy strategy = strategyClass.newInstance();
            ...
            if (ArrayUtil.isNotEmpty(annotations)) {
                final Optional<ICondition> conditionOptional = SensitiveConditions.getConditionOpt(annotations);
                if (conditionOptional.isNotPresent()
                        || conditionOptional.get().valid(context)) {
                    final Optional<IStrategy> strategyOptional = SensitiveStrategyBuiltInUtils.getStrategyOpt(annotations);
